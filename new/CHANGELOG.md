...
All notable changes to the Astro build under `new/`. Versions follow [semver](https://semver.org/).
Until cutover, production at `speak.junglestar.org` is still served by the Jekyll source under `old/` —
this changelog tracks `new/` only.
